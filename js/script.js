...
// Используем классы для карточек
    ...
    prev.addEventListener('click', () => { //При нажатии на стрелочку "влево", делаем все наоборот
        if (offset == 0) { //Если это первый слайд, то мы сдвигаем на последний
            offset = +width.slice(0, width.length - 2) * (slides.length - 1); //Если слайд последний, то сдвигается все на 0
        } else { //Если слайд не последний
            offset -= +width.slice(0, width.length - 2); //Отнимаем ширину слайда, на которую смещаем
        } 
